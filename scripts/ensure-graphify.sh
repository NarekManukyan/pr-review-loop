...
MARKER="$HOME/.claude/.pr-review-loop-graphify-checked"
[ "${1:-}" = "--force" ] && rm -f "$MARKER"
...
"$PY" -m pip install --user graphifyy >/dev/null 2>&1 && command -v graphify >/dev/null 2>&1 && {
  echo "[pr-review-loop] graphify installed via pip --user" >&2; exit 0; }
"$PY" -m pip install --user --break-system-packages graphifyy >/dev/null 2>&1 && command -v graphify >/dev/null 2>&1 && {
  echo "[pr-review-loop] graphify installed via pip --break-system-packages" >&2; exit 0; }

echo "[pr-review-loop] could not auto-install graphify — that's fine, JSONL recall still works." >&2
echo "[pr-review-loop] to add semantic recall later: pipx install graphifyy  (or uv tool install graphifyy)" >&2
exit 0
